...
/* Alle "geld"-state (saldo, posities, transacties) leeft hier, zodat Testing Mode
   en het Portfolio-venster altijd exact dezelfde live data zien — geen page-reload nodig. */
...
function tickerCategory(symbol) {
  return symbol === "BTC" ? "crypto" : "stocks";
}

function buildSyntheticPastYear(endValue, seed = 777, days = 365) {
  const rnd = seededRandom(seed);
  let v = endValue * 0.8;
  const raw = [];
  for (let i = 0; i < days; i++) {
    const drift = (rnd() - 0.47) * 0.017;
    v = Math.max(v * (1 + drift), v * 0.6);
    raw.push(v);
  }
  ...
  return raw.map((v, i) => ({
    idx: i - days,
    time: now - (days - i) * 24 * 3600 * 1000,
    value: Number((v * scale).toFixed(2)),
  }));
}
...
function PortfolioProvider({ children }) {
  ...
  const pastYear = useMemo(() => buildSyntheticPastYear(10000), []);
  ...
  const adviceBySymbol = useMemo(() => {
    const map = {};
    rows.forEach((r) => { map[r.ticker.symbol] = r.advice; });
    return map;
  }, [rows]);
  ...
  function deposit(amount) {
    ...
    if (!amt || amt <= 0) return;
    setCash((c) => c + amt);
    setStartingCapital((s) => s + amt);
    setTxs((list) => [{
      id: Date.now(), type: "deposit", symbol: "—", qty: 1, price: amt,
      time: new Date().toLocaleTimeString("nl-NL"), origin: "manual", reason: "Handmatige storting", confidence: null,
    }, ...list]);
  }

  function trade(symbol, type, qty) {
    const priceNow = prices[symbol];
    const cost = qty * priceNow;
    let profit = null;
    if (type === "buy") {
      if (cost > cashRef.current) return false;
      setCash((c) => c - cost);
      setHoldings((h) => {
        const cur = h[symbol] || { qty: 0, avg: 0 };
        const newQty = cur.qty + qty;
        const newAvg = (cur.avg * cur.qty + cost) / newQty;
        return { ...h, [symbol]: { qty: newQty, avg: newAvg } };
      });
    } else {
      const cur = holdingsRef.current[symbol];
      if (!cur || cur.qty < qty) return false;
      profit = (priceNow - cur.avg) * qty;
      setCash((c) => c + cost);
      setHoldings((h) => {
        const newQty = cur.qty - qty;
        const next = { ...h };
        if (newQty <= 0) delete next[symbol]; else next[symbol] = { ...cur, qty: newQty };
        return next;
      });
    }
    const advice = adviceBySymbol[symbol];
    setTxs((list) => [{
      id: Date.now(), type, symbol, qty, price: priceNow, time: new Date().toLocaleTimeString("nl-NL"),
      origin: "manual", profit, reason: "Handmatige order", confidence: advice ? advice.confidence : null,
    }, ...list]);
    return true;
  }
      ...
      if (advice.advice === "MOGELIJK KOPEN" && !cur) {
        ...
        if (q >= 1) {
          ...
          newTxs.push({
            ...
            reason: advice.reasons[0] || "AI-signaal", confidence: advice.confidence,
          });
        }
  ...
  useEffect(() => {
    setLiveHistory((h) => [...h, { idx: h.length, time: Date.now(), value: Number(totalValue.toFixed(2)) }].slice(-2000));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tick]);

  const realizedSells = txs.filter((t) => t.type === "sell" && typeof t.profit === "number");
  const openPositions = Object.entries(holdings).map(([sym, h]) => ({
    symbol: sym, profit: (prices[sym] - h.avg) * h.qty,
  }));
  ...
  function historyForDays(days) {
    if (days === 0) return liveHistory.length ? liveHistory : [{ idx: 0, time: Date.now(), value: totalValue }];
    const basePart = pastYear.slice(Math.max(0, pastYear.length - days));
    return [...basePart, ...liveHistory];
  }

  const value = {
    cash, startingCapital, holdings, txs, aiActive, setAiActive,
    prices, rows, adviceBySymbol, holdingsValue, totalValue, pnl, pnlPct,
    deposit, trade, evaluated, successRate, historyForDays, liveHistory,
  };
...
function TestingMode() {
  const {
    cash, startingCapital, holdings, txs, aiActive, setAiActive,
    prices, totalValue, pnl, pnlPct, deposit, trade, successRate, historyForDays,
  } = usePortfolio();
  ...
  const history = historyForDays(0).slice(-30);
